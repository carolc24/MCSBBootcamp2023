function [h,N,Time] = CF_Sim_ModLog2022(rate,K,initial,alpha,t,fullSimulation)
...
% ode45 method
if (fullSimulation)
   [T,X] = ode45(@(t,y) lambdam*y*(1-(y/theta).^alpha), 0:TimeStep:endTime, InitialPopulation);
   Time = T;
   N = X;
   %% Spline
   pp=spline(Time,N);
   h=ppval(pp,t);
    ...
    [T,X] = ode45(@(t,y) lambdam*y*(1-(y/theta).^alpha), t', InitialPopulation);
    h = X;
    N = X;
    Time = T;
end
